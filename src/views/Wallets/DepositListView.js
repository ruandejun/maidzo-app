...
import DepositItem from './component/DepositItem'
import { getBottomSpace } from 'react-native-iphone-x-helper';

const LIMIT = 20

class DepositListView extends React.Component {
    ...
    onRefresh(){
        ...
        this.setState({isFetching: true, loadingMore: false, canLoadMore: true}, () => {
            fetchApi('get', `page/get_data_deposit_by_username/${this.props.user.username}/data.json/`, {order: 'asc', offset: 0, limit: LIMIT})
            .then((data) => {
                // console.log(data)
    ...
    onEndReached(){
        ...
        this.setState({loadingMore: true}, () => {
            fetchApi('get', `page/get_data_deposit_by_username/${this.props.user.username}/data.json/`, {order: 'asc', offset: this.state.items.length, limit: LIMIT})
            .then((data) => {
                // console.log(data)
                if(data){
...
export default connect(mapStateToProps, mapDispatchToProps)(DepositListView);
